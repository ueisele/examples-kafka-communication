...
public class Verifier implements Runnable, AutoCloseable {
    ...
    protected Verifier(KafkaConsumer<String, String> consumer, String topic, boolean useAutoCommit, boolean useAsyncCommit, RecordFilter recordFilter, EventHandler eventHandler) {
        ...
        try {
            ...
            consumer.subscribe(Collections.singletonList(topic), new ConsumerRebalanceListener() {
                ...
                public void onPartitionsRevoked(Collection<TopicPartition> partitions) {
                    eventHandler.on(new PartitionsRevoked(partitions));
                }
                @Override
                public void onPartitionsAssigned(Collection<TopicPartition> partitions) {
                    eventHandler.on(new PartitionsAssigned(partitions));
                }
            });
    ...
    private Map<TopicPartition, OffsetAndMetadata> onRecordsReceived(ConsumerRecords<String, String> records) {
        ...
        eventHandler.on(new RecordsConsumed(totalRecords, totalAccepted, summaries));
        return offsets;
    }
    ...
    private RecordReceived verify(ConsumerRecord<String, String> record) {
        ...
        if(isInteger(record.value()) && isInSequence(record.key(), parseLong(record.value()))) {
            recordReceived = new ValidRecordReceived(record);
        } else {
            recordReceived = new UnexpectedRecordReceived(record);
        }
    ...
    public Map<String, RecordVerificationSummary> verificationSummary() {
        ...
        for (Map.Entry<String, List<String>> entry : new HashMap<>(valuesByKey).entrySet()) {
            ...
            summaries.put(key, new RecordVerificationSummary(key, values, duplicates, missing, invalid, outOfSequence));
        }
        return summaries;
    }
    ...
    private void onComplete(Map<TopicPartition, OffsetAndMetadata> offsets, Exception exception) {
        ...
        eventHandler.on(new OffsetsCommitted(committedOffsets, error, success));
    }
    ...
    public static class Builder {
        ...
        public Verifier build() {
            return new Verifier(consumer, topic, useAutoCommit, useAsyncCommit, new RecordFilter(keyFilterSupplier, valueFilterSupplier), eventHandler);
        }
    ...
    public static class ValidRecordReceived extends RecordReceived {
        ...
        public ValidRecordReceived(ConsumerRecord<String, String> record) {
            super(record);
        }
    ...
    public static class UnexpectedRecordReceived extends RecordReceived {
        ...
        public UnexpectedRecordReceived(ConsumerRecord<String, String> record) {
            super(record);
        }
    ...
    public static class RecordVerificationSummary extends ClientEvent {
        ...
        public RecordVerificationSummary(String key, long values, long duplicates, long missing, long invalid, boolean outOfSequence) {
            this.key = key;
            this.values = values;
    ...
    public static class RecordsConsumed extends ClientEvent {
        ...
        public RecordsConsumed(long count, long accepted, List<RecordSetSummary> partitionSummaries) {
            this.count = count;
    ...
    public static class RecordReceived extends ClientEvent {
        ...
        public RecordReceived(ConsumerRecord<String, String> record) {
            this.record = record;
        }
